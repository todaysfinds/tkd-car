...
class ScheduleLocation(db.Model):
    ...
    order = db.Column(db.Integer, nullable=False, default=0)  # 구역별 장소 순서
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    location = db.relationship('Location', backref=db.backref('schedule_locations', lazy=True))
    
    # 같은 구역 내에서 order는 고유값이어야 함 (데이터 정합성 보장)
    __table_args__ = (
        db.UniqueConstraint('day_of_week', 'session_part', 'type', 'order', 
                          name='uq_schedule_location_order'),
    )
...
def get_locations():
    try:
        ...
        # 3. 스케줄에서 사용 중인 장소들
        schedule_locations = db.session.query(ScheduleLocation.location_id).filter(
            ScheduleLocation.location_id.isnot(None)
        ).distinct().all()
